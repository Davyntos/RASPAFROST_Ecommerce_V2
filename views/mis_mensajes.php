...
<head>
    ...
    <title>Mis mensajes - RASPAFROST</title>
    ...
    <style>
        ...
        /* Estilos de la tabla de soporte */
        .table-responsive {
        ...
        .tabla-mensajes thead.table-dark {
        ...
        .respuesta-box {
            ...
            border-left: 3px solid var(--azul-frost) !important;
            border-radius: 8px !important;
            color: var(--gris-texto) !important;
            font-size: 13px !important;
...
<div class="container my-5">
    ...
    <div class="text-center mb-5">
        ...
        <h1 class="text-dark fw-bold m-0">✉️ Mis mensajes</h1>
        <p class="text-muted mt-2">
            Revisa el historial de tus solicitudes enviadas y las respuestas de nuestro equipo de atención.
        </p>
    </div>

    <div class="d-flex gap-2 mb-4">
        <a href="index.php" class="btn btn-secondary btn-custom shadow-sm">← Volver al catálogo</a>
        <a href="index.php?accion=mis_pedidos" class="btn btn-primary btn-custom shadow-sm">📦 Ver mis pedidos</a>
    </div>

    <?php if (empty($mensajes)): ?>
        
        <div class="alert alert-info text-center shadow-sm p-4 rounded-3">
            <h5 class="mb-1 fw-bold text-dark">Historial de soporte vacío</h5>
            <p class="mb-0 text-secondary">Aún no has enviado ninguna consulta o mensaje desde esta cuenta.</p>
        </div>

    <?php else: ?>

        <div class="table-responsive shadow-sm rounded-3">
            <table class="table table-bordered table-striped bg-white text-center align-middle mb-0 tabla-mensajes">
                <thead class="table-dark">
                    <tr>
                        <th style="width: 15%;">Fecha de envío</th>
                        <th style="width: 20%;">Asunto</th>
                        <th style="width: 30%;">Tu mensaje</th>
                        <th style="width: 12%;">Estado</th>
                        <th style="width: 23%;">Respuesta de atención</th>
                    </tr>
                </thead>
                <tbody>
                    ...
                    <tr>
                        <td class="text-muted small">
                            <?php 
                            $fecha_raw = $mensaje['fecha'] ?? $mensaje['created_at'] ?? null;
                            echo $fecha_raw ? htmlspecialchars(date('d/m/Y H:i', strtotime($fecha_raw))) : 'No disponible'; 
                            ?>
                        </td>

                        <td class="fw-semibold text-dark text-start px-3">
                            <?php echo htmlspecialchars($mensaje['asunto'] ?? 'Sin asunto'); ?>
                        </td>

                        <td class="text-start text-secondary px-3 text-break" style="font-size: 0.95rem;">
                            <?php echo htmlspecialchars($mensaje['mensaje'] ?? ''); ?>
                        </td>

                        <td>
                            <?php if (strtolower($mensaje['estado'] ?? '') === 'respondido' || !empty($mensaje['respuesta'])): ?>
                                <span class="badge bg-success badge-pill-custom">Respondido</span>
                            <?php else: ?>
                        ...
                        <td class="p-3">
                            <?php if (!empty($mensaje['respuesta'])): ?>
                                <div class="alert alert-success text-start mb-0 p-2 shadow-sm rounded-2 respuesta-box text-dark">
                                    <?php echo htmlspecialchars($mensaje['respuesta']); ?>
                                </div>
                            <?php else: ?>
                                <span class="text-muted text-italic small">Esperando revisión...</span>
                            <?php endif; ?>
                        </td>
                    </tr>
                    <?php endforeach; ?>
